...
class PDFMobileHandler {
  ...
  static Future<bool> handleDownload(
      Uint8List pdfBytes, Map<String, dynamic> reportData) async {
    ...
    try {
      ...
      if (shareSuccess) {
        ...
        print(
            '⚠️ El diálogo de compartir no se abrió, pero el archivo está guardado en: $filePath');
        return true;
      }
    } catch (e, stackTrace) {
  ...
  static Future<String?> _getAndroidDownloadsPath(String fileName) async {
    try {
      ...
      if (await altDownloadsDir.exists()) {
        ...
        print(
            '✅ Directorio de Descargas alternativo encontrado: ${altDownloadsDir.path}');
        return filePath;
      }
