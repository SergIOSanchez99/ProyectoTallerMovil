...
class MedicalRecommendationsService {
  ...
  static String generateRecommendations({
    ...
    } else if (resultLower.contains('pólipo') || resultLower.contains('polipo')) {
      return _getPolypRecommendations(stage, riskLower, confidence);
    } else if (resultLower.contains('anomalía') || resultLower.contains('anomalia')) {
      return _getAnomalyRecommendations(stage, riskLower, confidence);
    } else if (resultLower.contains('normal') || resultLower.contains('benigno') || resultLower.contains('sin anomalías')) {
  ...
  static String generateClinicalInterpretation({
    ...
    if (resultLower.contains('cáncer') || resultLower.contains('tumor')) {
      interpretation += 'Se detectaron hallazgos compatibles con neoplasia maligna en el colon. ';
      interpretation += 'El nivel de confianza del análisis es del $confidencePercent%. ';
      
      if (riskLower.contains('alto')) {
        interpretation += 'Se requiere atención médica inmediata y evaluación por oncología. ';
        interpretation += 'Se recomienda realizar estudios complementarios (biopsia, estadificación) ';
        interpretation += 'para confirmar el diagnóstico y determinar el estadio de la enfermedad.\n\n';
      } else {
        interpretation += 'Se recomienda evaluación urgente por especialista en gastroenterología ';
        interpretation += 'y oncología para confirmación diagnóstica y plan de tratamiento.\n\n';
      }
    } else if (resultLower.contains('pólipo') || resultLower.contains('polipo')) {
      ...
      if (riskLower.contains('alto') || riskLower.contains('medio')) {
        interpretation += 'Se recomienda evaluación endoscópica completa y posible polipectomía. ';
        interpretation += 'El análisis histopatológico determinará si se trata de pólipos adenomatosos ';
        interpretation += 'o hiperplásicos, lo cual influirá en el seguimiento.\n\n';
      } else {
        interpretation += 'Se recomienda seguimiento según las guías clínicas establecidas.\n\n';
      }
    } else if (resultLower.contains('anomalía') || resultLower.contains('anomalia')) {
      interpretation += 'Se detectaron alteraciones en la mucosa colónica que requieren evaluación. ';
      interpretation += 'El nivel de confianza del análisis es del $confidencePercent%. ';
      interpretation += 'Se recomienda correlación clínica y posible repetición del estudio ';
      interpretation += 'o evaluación endoscópica adicional para caracterización precisa.\n\n';
    } else {
      interpretation += 'No se detectaron anomalías significativas en el análisis. ';
      interpretation += 'El nivel de confianza del análisis es del $confidencePercent%. ';
      interpretation += 'Se recomienda seguimiento según protocolos de prevención establecidos.\n\n';
    }
    
    // Interpretación del nivel de confianza
    if (confidence >= 0.9) {
      interpretation += 'El nivel de confianza es alto, lo que sugiere alta precisión del análisis.';
    } else if (confidence >= 0.7) {
      interpretation += 'El nivel de confianza es moderado-alto. Se recomienda correlación con otros estudios.';
    } else if (confidence >= 0.5) {
      interpretation += 'El nivel de confianza es moderado. Se recomienda evaluación clínica adicional.';
    } else {
      interpretation += 'El nivel de confianza es bajo. Se recomienda repetición del estudio o evaluación complementaria.';
    }
  ...
  static String generateFollowUpPlan({
    ...
    if (resultLower.contains('cáncer') || resultLower.contains('tumor')) {
      plan += '1. Confirmación diagnóstica mediante biopsia y análisis histopatológico\n';
      plan += '2. Estadificación completa (TNM) mediante estudios de imagen\n';
      plan += '3. Evaluación multidisciplinaria (oncología, cirugía, gastroenterología)\n';
      plan += '4. Plan de tratamiento según estadio y características del paciente\n';
  ...
  static String _getCancerRecommendations(String stage, String riskLevel, double confidence) {
    ...
    if (riskLevel.contains('alto')) {
      ...
      recommendations += '• Realizar estudios complementarios (biopsia, estadificación)\n';
    ...
    if (confidence < 0.7) {
      recommendations += '• NOTA: El nivel de confianza es moderado. Se recomienda confirmación diagnóstica adicional.\n';
    }
  ...
  static String _getPolypRecommendations(String stage, String riskLevel, double confidence) {
    String recommendations = '';
  ...
  static String _getAnomalyRecommendations(String stage, String riskLevel, double confidence) {
    String recommendations = '';
  ...
  static String _getGeneralRecommendations(String riskLevel, double confidence) {
    String recommendations = '';
  ...
  static String generateComparativeAnalysis({
    ...
    final currentConfidence = _getConfidenceValue(currentReport['confidence'] ?? 0.0);
    final previousConfidence = _getConfidenceValue(previousReport['confidence'] ?? 0.0);
    ...
    if (currentResult == previousResult) {
      ...
      if (_isWorse(currentResult, previousResult)) {
        analysis += '• ⚠️ HAY EVIDENCIA DE PROGRESIÓN. Se requiere evaluación médica inmediata.\n';
      } else if (_isBetter(currentResult, previousResult)) {
        analysis += '• ✅ HAY EVIDENCIA DE MEJORÍA. Continuar con seguimiento.\n';
      }
    }
    
    analysis += '\nEVOLUCIÓN DEL RIESGO:\n';
    if (currentRisk == previousRisk) {
      analysis += '• El nivel de riesgo se mantiene: ${currentReport['riskLevel'] ?? currentReport['risk_level']}\n';
    } else {
      analysis += '• Riesgo previo: ${previousReport['riskLevel'] ?? previousReport['risk_level']}\n';
      analysis += '• Riesgo actual: ${currentReport['riskLevel'] ?? currentReport['risk_level']}\n';
      
      if (_isRiskHigher(currentRisk, previousRisk)) {
        analysis += '• ⚠️ AUMENTO DEL RIESGO DETECTADO. Se requiere atención médica.\n';
      } else {
    ...
    analysis += '• Confianza previa: ${(previousConfidence * 100).toStringAsFixed(1)}%\n';
    analysis += '• Confianza actual: ${(currentConfidence * 100).toStringAsFixed(1)}%\n';
    
    // Calcular intervalo entre estudios
    try {
      final currentDate = DateTime.tryParse(currentReport['date'] ?? currentReport['createdAt'] ?? '');
      final previousDate = DateTime.tryParse(previousReport['date'] ?? previousReport['createdAt'] ?? '');
      ...
      if (currentDate != null && previousDate != null) {
        ...
        if (months < 3) {
          ...
          analysis += '• Intervalo largo - considerar seguimiento más frecuente\n';
        }
      }
    } catch (e) {
